...
\end{equation}
subject to the collateral constraint $D' \leq \alpha_K K + \alpha_S S$, the dividend non-negativity constraint $\text{Div} \geq 0$, and the laws of motion for capital. This step is computationally expensive as it requires searching over $n_{I^K} \times n_{H^R} \times n_{D'}$ choice combinations at each state point.
...
\end{equation}
where starred variables denote the stored optimal policies. This step has computational complexity $O(n_z \times n_K \times n_S \times n_D)$ compared to $O(n_z \times n_K \times n_S \times n_D \times n_{I^K} \times n_{H^R} \times n_{D'})$ for full optimization.
