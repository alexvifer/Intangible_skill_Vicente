...
To ensure no default in equilibrium, banks lend only up to the recoverable collateral value at the risk-free rate. Banks evaluate borrowing capacity based on the firm's assets currently in place, leading to the collateral constraint:
\begin{equation}
D_{j,t} \le \alpha_K K_{j,t} + \alpha_S S_{j,t}.
\label{eq:collateral_constraint}
\end{equation}
This formulation captures that borrowing capacity depends on assets the lender can verify and seize, namely the capital stock at the time of lending.
...
\item \textbf{Financial decisions:} Firm simultaneously repays old debt \(RD_{t-1}\) and obtains new debt \(D_t\) from banks. The collateral constraint \eqref{eq:collateral_constraint} is based on current capital stocks \((K_t, S_t)\), which are observable to lenders at the time of borrowing. The net financial flow to the firm is \(D_t - RD_{t-1}\).
...
\begin{equation}
I^K_{j,t} + \text{AC}^K_{j,t} + \text{AC}^S_{j,t} + w_L L_{j,t} + w_H H^P_{j,t} + w_H H^R_{j,t} + R D_{j,t-1} \le Y_{j,t} + D_{j,t},
...
so that:
\begin{equation}
I^K_{j,t} + \text{AC}^K_{j,t} + \text{AC}^S_{j,t} + w_H H^R_{j,t} + R D_{j,t-1} \le \Pi^{gross}_{j,t} + D_{j,t}.
\label{eq:budget_constraint_gross}
\end{equation}

Dividends (profits) distributed to shareholders are:
\begin{equation}
\text{Div}_{j,t} = Y_{j,t} - I^K_{j,t} - \text{AC}^K_{j,t} - \text{AC}^S_{j,t} - w_L L_{j,t} - w_H(H^P_{j,t} + H^R_{j,t}) - R D_{j,t-1} + D_{j,t}.
\label{eq:dividends}
\end{equation}
...
\item Collateral constraint: \(D \le \alpha_K K + \alpha_S S\) (based on current capital)
\item Non-negative dividends: \(\text{Div} = \Pi^* - I^K - \text{AC}^K - \text{AC}^S - w_H H^R - R D_{-1} + D \ge 0\)
\end{itemize}

\textbf{Interpretation:} The collateral constraint depends on current capital stocks \((K, S)\), which are predetermined state variables. This means borrowing capacity is fixed at the beginning of the period: investment decisions do not affect current-period borrowing limits. Static production choices \((L, H^P)\) are independent of dynamic decisions and determined by first-order conditions given the state. 
...
\paragraph{Envelope Conditions.} Taking derivatives of \(V(z, K, S, D_{-1})\) with respect to state variables yields. Since the collateral constraint \eqref{eq:collateral_constraint} depends on current capital \((K, S)\), the collateral value appears directly:
\begin{equation}
V_K(z, K, S, D_{-1}) = \mu \frac{\partial Y}{\partial K} + \mu \frac{\phi_K}{2}\left(\frac{I^K}{K}\right)^2 + \lambda \alpha_K + (1-\delta_K)\beta(1-\zeta) \E[V_{K'}],
\label{eq:envelope_K}
\end{equation}
\begin{equation}
V_S(z, K, S, D_{-1}) = \mu \frac{\partial Y}{\partial S} + \mu \frac{\phi_S}{2}\left(\frac{\Delta S}{S}\right)^2 + \lambda \alpha_S + (1-\delta_S)\beta(1-\zeta) \E[V_{S'}],
\label{eq:envelope_S}
\end{equation}
\begin{equation}
...
\label{eq:envelope_D}
\end{equation}
The terms \(\mu \frac{\phi_K}{2}(I^K/K)^2\) and \(\mu \frac{\phi_S}{2}(\Delta S/S)^2\) reflect that higher capital reduces adjustment costs (for given investment). The collateral terms \(\lambda \alpha_K\) and \(\lambda \alpha_S\) enter directly since current capital determines borrowing capacity.

\paragraph{First-Order Condition for Tangible Investment.} From \(\partial \mathcal{L}/\partial I^K = 0\), accounting for adjustment costs:
\begin{equation}
\mu_{j,t} \left(1 + \phi_K \frac{I^K_{j,t}}{K_{j,t}}\right) = \beta(1-\zeta) \E[V_{K'}(z_{j,t+1}, K_{j,t+1}, S_{j,t+1}, D_{j,t}) \mid z_{j,t}].
\label{eq:foc_IK}
\end{equation}
The left-hand side is the marginal cost of investment: purchasing the unit plus the marginal adjustment cost, all valued at the shadow price \(\mu\). The right-hand side is the expected continuation value of capital. Since the collateral constraint depends on current capital (not next-period capital), investment does not directly relax the borrowing constraint within the period.

Combining \eqref{eq:foc_IK} with the envelope condition \eqref{eq:envelope_K}:
\begin{align}
\mu_{j,t} \left(1 + \phi_K \frac{I^K_{j,t}}{K_{j,t}}\right) &= \beta(1-\zeta) \E_{j,t}\Big[\mu_{j,t+1} \frac{\partial Y_{j,t+1}}{\partial K_{j,t+1}} + \mu_{j,t+1} \frac{\phi_K}{2}\left(\frac{I^K_{j,t+1}}{K_{j,t+1}}\right)^2 \nonumber \\
&\quad + \lambda_{j,t+1} \alpha_K + (1-\delta_K)\beta(1-\zeta) \E_{j,t+1}[V_{K',t+2}]\Big],
\label{eq:euler_K}
\end{align}
where \(\E_{j,t}[\cdot] \equiv \E[\cdot \mid z_{j,t}]\) denotes the conditional expectation given the firm's current productivity. The collateral term \(\lambda_{j,t+1}\alpha_K\) appears inside the expectation because capital invested today becomes collateral tomorrow.
...
\begin{equation}
\mu_{j,t} \left(w_H + \phi_S \frac{\Delta S_{j,t}}{S_{j,t}} \cdot \frac{\partial \Delta S_{j,t}}{\partial H^R_{j,t}}\right) = \beta(1-\zeta) \E[V_{S'}(z_{j,t+1}, K_{j,t+1}, S_{j,t+1}, D_{j,t}) \mid z_{j,t}] \cdot \Gamma \xi (H^R_{j,t})^{\xi-1}.
\label{eq:foc_HR}
\end{equation}
...
\begin{equation}
\mu_{j,t} w_H = \Gamma \xi (H^R_{j,t})^{\xi-1} \cdot \beta(1-\zeta) \E_{j,t}\left[\mu_{j,t+1} \frac{\partial Y_{j,t+1}}{\partial S_{j,t+1}} + \mu_{j,t+1} \frac{\phi_S}{2}\left(\frac{\Delta S_{j,t+1}}{S_{j,t+1}}\right)^2 + \lambda_{j,t+1} \alpha_S + (1-\delta_S)\mu_{j,t+1}\frac{w_{H,t+1}}{\Gamma\xi(H^R_{j,t+1})^{\xi-1}}\right].
\label{eq:euler_S}
\end{equation}
...
A constrained firm has \(\lambda > 0\), introducing wedges into both investment conditions. Because the collateral constraint is based on \emph{current} capital \((K, S)\), investment today does not relax the current-period borrowing constraint. However, today's investment becomes tomorrow's capital stock, which determines future borrowing capacity. From \eqref{eq:euler_K}, the term \(\lambda_{j,t+1} \alpha_K\) inside the expectation reflects that tangible capital accumulated today provides collateral value starting next period. Similarly, from \eqref{eq:euler_S}, the term \(\lambda_{j,t+1} \alpha_S\) provides an analogous (but smaller) future collateral benefit for intangible capital.

The crucial asymmetry arises from Assumption \ref{ass:pledgeability}: \(\alpha_K > \alpha_S\). Because tangible capital provides greater collateral value than intangible capital, constrained firms value the future collateral benefit of tangibles more than intangibles. This creates a pecking-order distortion where constrained firms tilt their investment composition toward tangibles, even when the fundamental marginal products would justify more intangible investment.
...
\end{equation}
where $\text{Div} = \Pi^*(z,K,S) - I^K - \text{AC}^K(I^K, K) - \text{AC}^S(\Delta S, S) - w_H H^R - R D_{-1} + D'$ includes adjustment costs $\text{AC}^K = (\phi_K/2)(I^K)^2/K$ and $\text{AC}^S = (\phi_S/2)(\Delta S)^2/S$. The optimization is subject to the collateral constraint $D' \leq \alpha_K K + \alpha_S S$ (based on current capital), the dividend non-negativity constraint $\text{Div} \geq 0$, and the laws of motion for capital $K' = (1-\delta_K)K + I^K$ and $S' = (1-\delta_S)S + \Gamma(H^R)^\xi$. This step is computationally expensive as it requires searching over $n_{I^K} \times n_{H^R} \times n_{D'}$ choice combinations at each state point.
