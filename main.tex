...
\begin{equation}
V_K = \zeta\left[\frac{\partial \pi}{\partial K} + (1-\delta_K)\right] + (1-\zeta)\left\{(1+\mu) \frac{\partial \pi}{\partial K} + (1+\mu) \frac{\phi_K}{2}\left(\frac{I^K}{K}\right)^2 + (1-\delta_K)\Big[\lambda \alpha_K + \beta \E[V_{K'}]\Big]\right\}.
\label{eq:envelope_K}
\end{equation}

Each envelope decomposes into an exit component (weighted by $\zeta$) and a continuation component (weighted by $1-\zeta$). The exit term $\partial\pi/\partial K + (1-\delta_K)$ is the marginal liquidation value of capital. In continuation, $(1+\mu)\frac{\phi_K}{2}(I^K/K)^2$ reflects that higher capital reduces adjustment costs, while $\lambda\alpha_K(1-\delta_K)$ captures the collateral benefit of undepreciated capital.

Combining \eqref{eq:foc_IK} and \eqref{eq:envelope_K} yields the Euler equation:
\begin{align}
(1 + \mu_t)\left(1 + \phi_K \frac{I^K_t}{K_t}\right) &= \lambda_t \alpha_K + \beta \E_t\Bigg[ \zeta\left(\frac{\partial \pi_{t+1}}{\partial K_{t+1}} + 1-\delta_K\right) \nonumber \\
&\quad + (1-\zeta)\bigg\{(1+\mu_{t+1}) \frac{\partial \pi_{t+1}}{\partial K_{t+1}} + (1+\mu_{t+1}) \frac{\phi_K}{2}\left(\frac{I^K_{t+1}}{K_{t+1}}\right)^2 \nonumber \\
&\quad + (1-\delta_K)\Big(\lambda_{t+1} \alpha_K + \beta \E_{t+1}[V_{K,t+2}]\Big)\bigg\}\Bigg].
\label{eq:euler_K}
\end{align}
...
\begin{equation}
V_S = \zeta\left[\frac{\partial \pi}{\partial S} + (1-\delta_S)\right] + (1-\zeta)\left\{(1+\mu) \frac{\partial \pi}{\partial S} + (1+\mu) \frac{\phi_S}{2}\left(\frac{\Delta S}{S}\right)^2 + (1-\delta_S)\Big[\lambda \alpha_S + \beta \E[V_{S'}]\Big]\right\}.
\label{eq:envelope_S}
\end{equation}
