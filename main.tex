...
\begin{abstract}
I develop a dynamic general-equilibrium model of heterogeneous firms where intangible capital and skilled labor are complements in production, and intangibles are produced with skilled labor in R\&D. Firms face collateal frictions, where tangible capital is more pledgeable than intangibles, creating a pecking-order distortion: constrained firms over-invest in tangibles. When the skilled-labor supply increases, unconstrained firms are able to exploit the complementarity investing in intangibles, while constrained firms do it suboptimally. The result is a \emph{skill-biased stagnation} mechanism: higher human capital delivers muted TFP gains because of misallocation induced by financial frictions.
\end{abstract}
...
I develop this argument in two stages. Using Portuguese matched employer-employee and balance sheet data for 2011--2022, I establish three empirical facts: (1) intangible intensity and skilled labor share are complements in production, (2) financially constrained firms invest less in intangibles and R\&D labor, and (3) the complementarity between intangibles and skills is significantly weaker for constrained firms. I then build a quantitative heterogeneous-firm model with nested CES production technology, endogenous intangible capital accumulation through R\&D, and collateral constraints with differential pledgeability. The model rationalizes the empirical patterns and enables counterfactual analysis of skill supply shocks under different financial friction scenarios.
...
I define skilled workers as those with tertiary education (ISCED 5-8) and construct the share of skilled workers as the key skill measure. Intangible capital stocks are built using the perpetual inventory method following \citet{peterstaylor2017}, combining knowledge capital from R\&D expenditures, organization capital from SG\&A expenses (advertising and training), and balance sheet intangibles. All monetary variables are deflated to constant 2020 prices. After cleaning and sample restrictions (positive employment, non-negative balance sheet items, at least two consecutive observations per firm), the final analytical sample contains 1,759,093 firm-year observations. Details on data construction and cleaning are provided in Appendix \ref{app:data_cleaning}.
...
Table \ref{tab:complementarity} reports estimates from production function regressions where the dependent variable is log output (revenue, production value, or gross value added) and the key regressor is the interaction between intangible intensity (intangible capital divided by total capital) and the share of skilled workers. All specifications include firm, year, and industry fixed effects, controlling for log total capital, log employment, and log firm age.

\input{QP_SCIE/results/fact1_complementarity.tex}

The interaction coefficient is positive and statistically significant across all output measures, ranging from 0.026 (revenue) to 0.042 (production value). This indicates that the marginal product of skilled labor is increasing in intangible intensity: firms with higher intangible capital benefit more from employing skilled workers. Conversely, the marginal product of intangibles is increasing in skill share. This complementarity is precisely the technological structure embedded in the nested CES production function of the quantitative model.
...
If financial constraints prevent firms from building intangible capital, they should also reduce the extent to which firms can exploit the technological complementarity between intangibles and skills. Table \ref{tab:underexploitation} tests this prediction by estimating the complementarity regression separately for low-leverage (unconstrained) and high-leverage (constrained) firms.

\input{QP_SCIE/results/fact3_underexploitation.tex}

The interaction coefficient is 0.055 and significant for low-leverage firms (column 2), but statistically indistinguishable from zero ($-0.006$, p $>$ 0.10) for high-leverage firms (column 3). This stark difference indicates that constrained firms cannot exploit the complementarity: even when they employ skilled workers, the lack of complementary intangible capital limits productivity gains. The pooled specification (column 1) masks this heterogeneity, showing an intermediate coefficient of 0.040.

Figure \ref{fig:fact3_wage_premium} provides additional evidence by examining wage premia (the ratio of skilled to unskilled wages). If constrained firms underexploit skilled labor due to insufficient intangible capital, skilled workers should command lower premia at constrained firms. The residualized binscatter confirms this prediction: wage premia decline with leverage, consistent with lower marginal products of skilled labor at constrained firms.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{QP_SCIE/results/fact3_wage_premium_leverage.png}
\caption{Wage Premium and Financial Constraints}
\label{fig:fact3_wage_premium}
\note{Wage premium defined as the ratio of average skilled to unskilled wages within the firm. Variables residualized by regressing on firm, year, and industry fixed effects, log total capital, log employment, and log firm age.}
\end{figure}

\subsection{Summary of Empirical Evidence}

The three facts establish the key building blocks of the theoretical mechanism. First, technology features complementarity between intangibles and skills. Second, financial frictions distort investment composition toward tangibles and away from intangibles. Third, the combination of these forces prevents constrained firms from exploiting complementarity, reducing the aggregate returns to skill accumulation. The following sections develop a model that rationalizes these patterns and enables quantitative counterfactual analysis.
...
To clarify the sequence of decisions within each period \(t\), I specify the following timing:
