...
\label{eq:adj_cost_K}
\end{equation}
where \(\phi_K \geq 0\) governs the magnitude of adjustment costs. Setting \(\phi_K = 0\) nests the frictionless investment case.

\begin{remark}[Role of Adjustment Costs]
Adjustment costs serve two purposes. First, they make capital a genuinely dynamic state variable, preventing instantaneous convergence to the frictionless optimum. Second, they create an incentive for firms to smooth investment over time, which interacts with borrowing constraints: adjustment costs make capital valuable as collateral because firms cannot instantly adjust their capital stock in response to binding constraints.
\end{remark}
...
To ensure no default in equilibrium, banks lend only up to the recoverable collateral value at the risk-free rate. Since debt is repaid at the beginning of the next period, lenders care about the collateral value at repayment time---namely, next period's capital stocks. Given the timing structure described below, the collateral constraint is:
\begin{equation}
D_{j,t} \le \alpha_K K_{j,t+1} + \alpha_S S_{j,t+1}.
\label{eq:collateral_constraint}
\end{equation}
This formulation captures that borrowing capacity depends on the capital the firm will have when the loan comes due, not the capital it currently holds.
...
\item \textbf{Financial decisions:} Firm simultaneously repays old debt \(RD_{t-1}\) and obtains new debt \(D_t\) from banks. Since debt is due at the beginning of period \(t+1\), the collateral constraint \eqref{eq:collateral_constraint} is based on next period's capital stocks \((K_{t+1}, S_{t+1})\), which depend on today's investment decisions. In practice, the firm chooses debt and investment jointly to satisfy this constraint. The net financial flow to the firm is \(D_t - RD_{t-1}\).
...
\begin{equation}
I^K_{j,t} + \text{AC}^K_{j,t} + w_L L_{j,t} + w_H H^P_{j,t} + w_H H^R_{j,t} + R D_{j,t-1} \le Y_{j,t} + D_{j,t},
\label{eq:budget_constraint}
\end{equation}
where \(\text{AC}^K_{j,t}\) is the adjustment cost from \eqref{eq:adj_cost_K}.
...
so that:
\begin{equation}
I^K_{j,t} + \text{AC}^K_{j,t} + w_H H^R_{j,t} + R D_{j,t-1} \le \Pi^{gross}_{j,t} + D_{j,t}.
\label{eq:budget_constraint_gross}
\end{equation}

Dividends (profits) distributed to shareholders are:
\begin{equation}
\text{Div}_{j,t} = Y_{j,t} - I^K_{j,t} - \text{AC}^K_{j,t} - w_L L_{j,t} - w_H(H^P_{j,t} + H^R_{j,t}) - R D_{j,t-1} + D_{j,t}.
\label{eq:dividends}
\end{equation}
...
\begin{align}
V(z, K, S, D_{-1}) &= \max_{D, I^K, H^R} \Bigg\{ \Pi^*(z, K, S) - R D_{-1} + D \nonumber \\
&\quad - I^K - \text{AC}^K(I^K, K) - w_H H^R + \beta(1-\zeta) \E[V(z', K', S', D) \mid z] \Bigg\},
...
\item Non-negative dividends: \(\text{Div} = \Pi^* - I^K - \text{AC}^K - w_H H^R - R D_{-1} + D \ge 0\)
\end{itemize}

\textbf{Interpretation:} The firm chooses debt \(D\) and investment \((I^K, H^R)\) jointly, since the collateral constraint depends on next-period capital stocks \((K', S')\) which are determined by investment. This creates an interaction between investment and financing decisions: higher investment expands borrowing capacity (through \(\alpha_K K' + \alpha_S S'\)), but also requires more resources. Static production choices \((L, H^P)\) are independent of these dynamic decisions and determined by first-order conditions given the state. 
...
\paragraph{Envelope Conditions.} Taking derivatives of \(V(z, K, S, D_{-1})\) with respect to state variables yields. Since the collateral constraint \eqref{eq:collateral_constraint} depends on next-period capital \((K', S')\), current capital \((K, S)\) affects collateral value only through undepreciated capital:
\begin{equation}
V_K(z, K, S, D_{-1}) = \mu \frac{\partial Y}{\partial K} + \mu \frac{\phi_K}{2}\left(\frac{I^K}{K}\right)^2 + (1-\delta_K)\Big[\beta(1-\zeta) \E[V_{K'}] + \lambda \alpha_K\Big],
\label{eq:envelope_K}
\end{equation}
\begin{equation}
V_S(z, K, S, D_{-1}) = \mu \frac{\partial Y}{\partial S} + (1-\delta_S)\Big[\beta(1-\zeta) \E[V_{S'}] + \lambda \alpha_S\Big],
\label{eq:envelope_S}
\end{equation}
\begin{equation}
...
\label{eq:envelope_D}
\end{equation}
The term \(\mu \frac{\phi_K}{2}(I^K/K)^2\) in \eqref{eq:envelope_K} reflects that higher \(K\) reduces adjustment costs (for given \(I^K\)). The collateral terms \(\lambda \alpha_K\) and \(\lambda \alpha_S\) are multiplied by \((1-\delta_K)\) and \((1-\delta_S)\) respectively, since undepreciated capital enters next period's collateral.

\paragraph{First-Order Condition for Tangible Investment.} From \(\partial \mathcal{L}/\partial I^K = 0\), accounting for adjustment costs and the collateral constraint on \(K'\):
\begin{equation}
\mu_{j,t} \left(1 + \phi_K \frac{I^K_{j,t}}{K_{j,t}}\right) = \beta(1-\zeta) \E[V_{K'}(z_{j,t+1}, K_{j,t+1}, S_{j,t+1}, D_{j,t}) \mid z_{j,t}] + \lambda_{j,t} \alpha_K.
\label{eq:foc_IK}
\end{equation}
The left-hand side is the marginal cost of investment: purchasing the unit plus the marginal adjustment cost, all valued at the shadow price \(\mu\). The right-hand side includes the continuation value of capital plus the current-period benefit of relaxing the collateral constraint (\(\lambda \alpha_K\)).
...
&\quad + (1-\delta_K)\big(\beta(1-\zeta) \E_{j,t+1}[V_{K',t+2}] + \lambda_{j,t+1} \alpha_K\big)\Big],
\label{eq:euler_K}
\end{align}
where \(\E_{j,t}[\cdot] \equiv \E[\cdot \mid z_{j,t}]\) denotes the conditional expectation given the firm's current productivity. The adjustment cost terms create an additional incentive to smooth investment over time.

\paragraph{First-Order Condition for Intangible Investment (via R\&D).} The firm chooses \(H^R\) to balance the wage cost against the marginal value of additional intangible capital. Since \(S' = (1-\delta_S)S + \Gamma(H^R)^{\xi}\), we have \(\partial S'/\partial H^R = \Gamma \xi (H^R)^{\xi-1}\). With collateral based on next-period capital, increasing \(H^R\) also relaxes the current borrowing constraint by \(\lambda \alpha_S \partial S'/\partial H^R\).

The FOC with respect to \(H^R\) is:
\begin{equation}
\mu_{j,t} w_H = \Big[\beta(1-\zeta) \E[V_{S'}(z_{j,t+1}, K_{j,t+1}, S_{j,t+1}, D_{j,t}) \mid z_{j,t}] + \lambda_{j,t} \alpha_S\Big] \cdot \Gamma \xi (H^R_{j,t})^{\xi-1}.
\label{eq:foc_HR}
\end{equation}

Using the envelope condition \eqref{eq:envelope_S} and substituting recursively, we obtain:\footnote{Undepreciated intangible capital must be valued at its marginal replacement cost $q_{S,t+1} \equiv w_{H,t+1}/[\Gamma\xi(H^R_{j,t+1})^{\xi-1}]$, not at unity. From the period $t+1$ FOC for $H^R_{t+1}$, we have $\beta(1-\zeta)\mathbb{E}_{t+1}[V_{S,t+2}] + \lambda_{t+1}\alpha_S = \mu_{t+1}w_{H,t+1}/[\Gamma\xi(H^R_{j,t+1})^{\xi-1}]$. Substituting this expression into the envelope condition yields the correct continuation value term.}
\begin{equation}
\mu_{j,t} w_H = \Gamma \xi (H^R_{j,t})^{\xi-1} \left[\lambda_{j,t} \alpha_S + \beta(1-\zeta) \E_{j,t}\left[\mu_{j,t+1} \frac{\partial Y_{j,t+1}}{\partial S_{j,t+1}} + (1-\delta_S)\mu_{j,t+1}\frac{w_{H,t+1}}{\Gamma\xi(H^R_{j,t+1})^{\xi-1}}\right]\right].
\label{eq:euler_S}
\end{equation}
...
A constrained firm has \(\lambda > 0\), introducing wedges into both investment conditions. Because the collateral constraint is based on \emph{next-period} capital \((K', S')\), investment today \emph{immediately} relaxes the current borrowing constraint. From \eqref{eq:foc_IK}, the term \(\lambda_{j,t} \alpha_K\) on the right-hand side reduces the effective cost of tangible investment: each unit of \(I^K\) allows the firm to borrow an additional \(\alpha_K\) today. Similarly, from \eqref{eq:foc_HR}, the term \(\lambda_{j,t} \alpha_S\) provides an analogous (but smaller) benefit for R\&D investment.

The crucial asymmetry arises from Assumption \ref{ass:pledgeability}: \(\alpha_K > \alpha_S\). When the collateral constraint binds, tangible capital provides greater collateral value than intangible capital, \emph{immediately} in the current period. This creates a pecking-order distortion where constrained firms tilt their investment composition toward tangibles, even when the fundamental marginal products would justify more intangible investment. The adjustment costs \(\phi_K\) reinforce the dynamic nature of capital, making collateral value more persistent.
...
\paragraph{State Variables.} The firm's state is $(z, K, S, D_{-1})$ where $z$ is idiosyncratic productivity, $K$ is tangible capital, $S$ is intangible capital, and $D_{-1}$ is beginning-of-period debt. Productivity $z$ is discretized using the \citet{tauchen1986} method with $n_z$ grid points covering $\pm 3$ unconditional standard deviations. The continuous state variables $(K, S, D)$ are discretized on grids:
\begin{itemize}
...
\paragraph{Choice Variables.} The firm chooses tangible investment $I^K$, R\&D labor $H^R$, and new debt $D'$. Static labor inputs $(L, H^P)$ are solved analytically from first-order conditions given the state and wages. The dynamic choices are discretized on grids:
\begin{itemize}
...
\item New debt $D'$: $n_{D'}$ points from 0 to $D_{\max}$
\end{itemize}
...
\begin{equation}
V^{new}(z, K, S, D_{-1}) = \max_{D', I^K, H^R} \left\{ \text{Div} + \beta(1-\zeta) \mathbb{E}_{z'|z}\left[ V(z', K', S', D') \right] \right\}
\end{equation}
where $\text{Div} = \Pi^*(z,K,S) - I^K - \text{AC}^K(I^K, K) - w_H H^R - R D_{-1} + D'$ includes the adjustment cost $\text{AC}^K = (\phi_K/2)(I^K)^2/K$. The optimization is subject to the collateral constraint $D' \leq \alpha_K K' + \alpha_S S'$ (based on next-period capital), the dividend non-negativity constraint $\text{Div} \geq 0$, and the laws of motion for capital $K' = (1-\delta_K)K + I^K$ and $S' = (1-\delta_S)S + \Gamma(H^R)^\xi$. This step is computationally expensive as it requires searching over $n_{I^K} \times n_{H^R} \times n_{D'}$ choice combinations at each state point.
...
\begin{equation}
V^{new}(z, K, S, D_{-1}) = \text{Div}^*(z, K, S, D_{-1}) + \beta(1-\zeta) \mathbb{E}_{z'|z}\left[ V(z', K'^*, S'^*, D'^*) \right]
\end{equation}
where starred variables denote the stored optimal policies, and $\text{Div}^*$ is computed using these policies (including the associated adjustment costs). This step has computational complexity $O(n_z \times n_K \times n_S \times n_D)$ compared to $O(n_z \times n_K \times n_S \times n_D \times n_{I^K} \times n_{H^R} \times n_{D'})$ for full optimization.
...
\paragraph{2. Local Search.} After the first VFI iteration, the optimal policy at each state typically changes only slightly. Instead of searching the entire choice grid, subsequent policy improvement steps search only within $\pm 3$ grid points of the previous optimum. This reduces the choice space from $n_{I^K} \times n_{H^R} \times n_{D'} = 10{,}000$ combinations to $(2 \times 3 + 1)^3 = 343$ combinations, a 29$\times$ reduction.

\paragraph{3. Precomputed Static Labor.} The static labor choices $(L^*, H^{P*})$ depend only on $(z, K, S)$ and wages---not on debt $D_{-1}$. Before each policy improvement step, optimal static labor and associated output $Y^*$ and gross profits $\Pi^*$ are precomputed for all $(z, K, S)$ triplets. This reduces redundant computation by a factor of $n_D$.
...
The model is implemented in Fortran 90/95 for computational efficiency, using double precision arithmetic throughout. The adjustment cost parameter is set to $\phi_K = 2.0$ in the baseline calibration, consistent with estimates in the investment literature \citep{cooper2006dynamics}. Key numerical parameters:
\begin{center}
\begin{tabular}{lcc}
\toprule
