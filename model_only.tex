...
\item \textbf{Investment, financing, and repayment (survivors):} With probability $(1-\zeta)$, the firm survives. The firm repays maturing debt $(1+r)D_t$, chooses tangible investment $I^K_t$, R\&D labor $H^R_t$, and new debt $D_{t+1}$ subject to the collateral constraint \eqref{eq:collateral_constraint} and the non-negative dividend constraint.
...
\begin{equation}
\text{Div} = \pi(z, K, S) - I^K - w_H H^R - (1+r) D + D'.
\label{eq:dividends}
\end{equation}
...
subject to:
\begin{itemize}
\item Dividends: $\text{Div} = \pi(z, K, S) - I^K - w_H H^R - (1+r) D + D'$
...
\]
The net financing need is $D_{\text{needed}} = \text{expenses} - \text{cash flow}$. Given that $\beta(1+r) = 1$, the net value of an additional dollar of debt is $-(1-\zeta)\E[\mu'] \le 0$, reflecting the expected future shadow cost of funds. Thus the firm never borrows more than necessary. The solution is:
\begin{itemize}
...
\label{eq:foc_D}
\end{equation}
With $\beta(1+r) = 1$, this simplifies to $\mu = \lambda + (1-\zeta)\E[\mu']$. For an unconstrained firm ($\mu = \lambda = 0$), this yields $\E[\mu'] = 0$: unconstrained firms expect to remain unconstrained.

\paragraph{FOC for Tangible Investment ($I^K$).}
\begin{equation}
(1 + \mu) = \lambda \alpha_K + \beta \E[V_{K'}].
\label{eq:foc_IK}
\end{equation}

The left-hand side is the marginal cost of purchasing one unit of capital, valued at $(1+\mu)$ reflecting the shadow cost of internal funds. The right-hand side has two components:
\begin{enumerate}
...
\item $\beta \E[V_{K'}]$: the expected marginal value of additional capital, incorporating both the liquidation return in exit states and the continuation value for survivors.
\end{enumerate}

\paragraph{Envelope Condition for $K$.}
\begin{equation}
V_K = \zeta\left[\frac{\partial \pi}{\partial K} + (1-\delta_K)\right] + (1-\zeta)\left\{(1+\mu) \frac{\partial \pi}{\partial K} + (1-\delta_K)\Big[\lambda \alpha_K + \beta \E[V_{K'}]\Big]\right\}.
\label{eq:envelope_K}
\end{equation}

Each envelope decomposes into an exit component (weighted by $\zeta$) and a continuation component (weighted by $1-\zeta$). The exit term $\partial\pi/\partial K + (1-\delta_K)$ is the marginal liquidation value of capital. In continuation, $\lambda\alpha_K(1-\delta_K)$ captures the collateral benefit of undepreciated capital.

Combining \eqref{eq:foc_IK} and \eqref{eq:envelope_K} yields the Euler equation:
\begin{align}
(1 + \mu_t) &= \lambda_t \alpha_K + \beta \E_t\Bigg[ \zeta\left(\frac{\partial \pi_{t+1}}{\partial K_{t+1}} + 1-\delta_K\right) \nonumber \\
&\quad + (1-\zeta)\bigg\{(1+\mu_{t+1}) \frac{\partial \pi_{t+1}}{\partial K_{t+1}} + (1-\delta_K)\Big(\lambda_{t+1} \alpha_K + \beta \E_{t+1}[V_{K,t+2}]\Big)\bigg\}\Bigg].
\label{eq:euler_K}
\end{align}

\paragraph{FOC for Intangible Investment (via $H^R$).} Since $\partial S'/\partial H^R = \Gamma \xi (H^R)^{\xi-1}$:
\begin{equation}
(1+\mu) \, w_H = \Big[\lambda \alpha_S + \beta \E[V_{S'}]\Big] \cdot \Gamma \xi (H^R)^{\xi-1}.
\label{eq:foc_HR}
\end{equation}

Defining the marginal cost of producing intangibles as $q_S \equiv w_H / [\Gamma \xi (H^R)^{\xi-1}]$, this can be written as:
\begin{equation}
...
\begin{equation}
V_S = \zeta\left[\frac{\partial \pi}{\partial S} + (1-\delta_S)\right] + (1-\zeta)\left\{(1+\mu) \frac{\partial \pi}{\partial S} + (1-\delta_S)\Big[\lambda \alpha_S + \beta \E[V_{S'}]\Big]\right\}.
\label{eq:envelope_S}
\end{equation}

\paragraph{Envelope Condition for $D$.}
\begin{equation}
V_D = -(1+r)\big[\zeta + (1-\zeta)(1+\mu)\big].
\label{eq:envelope_D}
\end{equation}
...
\paragraph{Capital Composition Distortion.} Rearranging the FOCs:
\begin{equation}
\frac{\text{MB}_K - \lambda \alpha_K}{\text{MB}_S - \lambda \alpha_S} = \frac{1}{q_S},
\label{eq:capital_ratio}
\end{equation}
where $\text{MB}_K \equiv \beta\E[V_{K'}]$ and $\text{MB}_S \equiv \beta\E[V_{S'}]$ denote the expected marginal value terms (including both liquidation and continuation returns). For constrained firms, since $\alpha_K > \alpha_S$, the numerator is reduced more than the denominator, distorting capital composition toward tangibles.
...
\[
C + \int I^K_j \, d\Psi^* = \int Y_j \, d\Psi^* + \zeta \int \big[(1-\delta_K)K_j + (1-\delta_S)S_j\big] d\Psi^* - \zeta (K_0 + S_0).
\]
